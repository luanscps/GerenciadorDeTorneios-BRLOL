...
**Versão:** 0.2.0
**Stack:** Next.js 16.2.6 (App Router) + React 19.0.1 + TypeScript ^5 + Supabase SSR + Tailwind CSS + Zod + React Hook Form
...
## 5. Fluxo de Dados e Padrões de Busca
...
- `tournaments` — torneios (slug, status, bracket_type, max_teams, organizer_id, discord_webhook_url)
...
- `disputes` — disputas de partidas (match_id, reason, status, evidence_url)
- `audit_log` — log de ações admin (action, table_name, old_data, new_data)
- `seedings` — seedings de times por torneio
- `prize_distribution` — premiações por placement no torneio
- `site_terms_acceptance` — aceite de termos pelos usuários
- `tournament_match_results` — resultados recebidos via callback Riot (tournament_code, game_data)
- `tournament_rules` — regras por torneio (section, content, rule_order)
- `active_team` — time ativo do perfil (profile_id → team_id, 1:1)

**Views:**
- `v_player_leaderboard` — ranking geral com KDA, MVP count, tier, LP
- `v_stage_standings` — standings por fase (wins, losses, points, team)
- `v_player_tournament_kda` — KDA médio por torneio por jogador

**Funções RPC:**
- `accept_team_invite(p_invite_id, p_profile_id, p_riot_acc_id?)` → Json
- `is_admin(uid)` → boolean
- `is_current_user_admin()` → boolean
- `is_organizer_or_admin(uid)` → boolean
- `is_tournament_organizer(tid, uid)` → boolean
- `log_admin_action(p_action, p_table_name, ...)` → void
...
## 7. Integração Riot API — Regras Específicas
