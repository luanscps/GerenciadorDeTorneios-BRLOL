...
export default function FasesPage() {
  ...
  function openEdit(stage: Stage) {
    setForm({
      name:         stage.name,
      bracket_type: stage.bracket_type,
      best_of:      stage.best_of,
      stage_order:  stage.stage_order,
      slots:        stage.slots?.toString() ?? "",
    });
    setEditId(stage.id);
    setShowForm(true);
    setError("");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = editId
      ? await fetch(`/api/admin/tournaments/${id}/stages`, {
          method: "PATCH",
    ...
    if (!res.ok || json.error) {
      setError(json.error ?? "Erro ao salvar fase");
    } else {
      setSuccess(editId ? "Fase atualizada!" : "Fase criada!");
      setShowForm(false);
      setEditId(null);
      fetchStages();
    }
  ...
  async function handleDelete(stageId: string) {
    ...
    setDeleting(stageId);
    const res = await fetch(`/api/admin/tournaments/${id}/stages?stageId=${stageId}`, {
      method: "DELETE",
    });
    const json = await res.json();
    if (!res.ok || json.error) {
      setError(json.error ?? "Erro ao deletar fase");
    } else {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <Link href={`/admin/tournaments/${id}`} className="text-gray-400 hover:text-white text-sm">
            ← Voltar ao torneio
          </Link>
          <h1 className="text-2xl font-bold text-white mt-1">Fases do Torneio</h1>
          <p className="text-gray-400 text-sm mt-0.5">
            Configure as etapas da competição — grupos, mata-mata, gauntlet, etc.
          </p>
        </div>
        {!showForm && (
          <button onClick={openCreate} className="btn-gold px-4 py-2 text-sm">
            + Nova Fase
          </button>
        )}
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Fase de Grupos, Semifinal, Grande Final"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="input-lol w-full bg-[#0A1428]"
                >
                  {BRACKET_TYPES.map(b => (
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2].map(i => (
            <div key={i} className="bg-[#0D1B2E] border border-[#1E3A5F] rounded-lg p-4 animate-pulse">
              <div className="h-4 bg-[#1E3A5F] rounded w-1/3 mb-2" />
        ...
        <div className="card-lol text-center py-12">
          ...
          <p className="text-gray-400 text-sm mb-4">
            Crie as etapas do seu torneio — grupos, playoff, grande final.
          </p>
          <button onClick={openCreate} className="btn-gold px-4 py-2 text-sm">
            + Criar primeira fase
          </button>
        ...
        <div className="space-y-3">
          {stages
            ...
            .map((stage) => (
              <div
                key={stage.id}
                className="bg-[#0D1B2E] border border-[#1E3A5F] rounded-lg p-4 flex items-center justify-between gap-4 flex-wrap"
              >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      <span className="text-gray-400 text-xs">
                        {BRACKET_LABEL[stage.bracket_type] ?? stage.bracket_type}
                      </span>
                      ...
                      {stage.slots && (
                        <>
                          <span className="text-gray-600 text-xs">·</span>
                          <span className="text-gray-400 text-xs">{stage.slots} vagas</span>
                        </>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`text-xs px-2 py-0.5 rounded border ${
                      STATUS_COLOR[stage.status] ?? "text-gray-400 bg-gray-400/10 border-gray-400/20"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className="text-xs text-[#C8A84B] hover:underline px-2 py-1"
                  >
                    Editar
                  </button>
                  <button
